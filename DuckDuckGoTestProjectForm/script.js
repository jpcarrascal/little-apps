...
// Toggle between print and edit view
function togglePrintView(button) {
    const body = document.body;
    const isPrintMode = body.classList.contains('print-mode');
    
    if (isPrintMode) {
        // Switch to Edit View
        body.classList.remove('print-mode');
        button.textContent = 'Print View';
    } else {
        // Switch to Print View
        body.classList.add('print-mode');
        button.textContent = 'Edit View';
        
        // Update print divs with current textarea content
        updatePrintContent();
    }
}

// Update print content divs with textarea values
function updatePrintContent() {
    for (let i = 0; i < 6; i++) {
        const textarea = document.getElementById(`field_${i}`);
        let printDiv = document.getElementById(`print_${i}`);
        
        // Create print div if it doesn't exist
        if (!printDiv) {
            printDiv = document.createElement('div');
            printDiv.id = `print_${i}`;
            printDiv.className = 'print-content';
            textarea.parentNode.insertBefore(printDiv, textarea);
        }
        
        // Set content, add empty class if needed
        const content = textarea.value.trim();
        if (content) {
            printDiv.textContent = content;
            printDiv.classList.remove('empty-content');
        } else {
            printDiv.textContent = '';
            printDiv.classList.add('empty-content');
        }
    }
}
